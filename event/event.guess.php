<?php
	...
	class Guess {
		...
		// 添加选择
		public function selectAdd ( $token, $tag, $select, $number=1) {
			$sql = "insert INTO `game`.`game_select` (`id`, `user`, `select`, `time`, `tag`, `number`) VALUES (null, '".$token."', '".$select."', '".time()."', '".$tag."', ".$number.");";
			$query = mysql_query($sql);
			// print_r($query);
		}
